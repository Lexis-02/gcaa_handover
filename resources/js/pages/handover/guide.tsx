...
import { AlertCircle, ArrowRight, CheckCircle2, FileText, Pen } from 'lucide-react';
...
export default function HandoverGuide({
    ...
    return (
        <>
            ...
            >
                {/* Page Header */}
                <motion.div variants={pageItem} className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-primary">How it works</p>
                        <h1 className="text-2xl font-semibold tracking-tight text-foreground md:text-3xl">
                            PC Handover Guide
                        </h1>
                        <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
                            Every PC passes through three sequential sign-off stages before it is marked{' '}
                            <strong className="font-medium text-foreground">Complete</strong>.
                        </p>
                    </div>
                    {can_access_sign_offs && (
                ...
                <motion.div variants={pageItem}>
                    <div className="relative">
                        {/* Connecting line */}
                        <div className="absolute left-[18px] top-10 h-[calc(100%-5rem)] w-px bg-border" />
                        ...
                        <ol className="space-y-0">
                            ...
                            <li className="relative flex items-center gap-4 pb-8">
                                ...
                                <div>
                                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Registered</p>
                                    <p className="text-sm text-muted-foreground">ICT Admin creates the PC record and the handover enters the pipeline.</p>
                                </div>
                            </li>

                            {/* Stage steps */}
                            {stages.map((stage, index) => (
                                <li key={stage.key} className="relative flex gap-4 pb-8">
                                    {/* Step number bubble */}
                            ...
                            <li className="relative flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">All stages signed. The record is archived.</p>
                                </div>
                            </li>
                        </ol>
                    </div>
                ...
                <motion.div variants={pageItem} className="flex gap-3 rounded-xl border border-border/60 bg-card p-5 shadow-sm">
                    <AlertCircle className="mt-0.5 size-4 shrink-0 text-muted-foreground" strokeWidth={1.75} />
                    <div>
                        <h2 className="font-semibold text-foreground">Who can delete a register record?</h2>
                        <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                            Only <strong className="font-medium text-foreground">ICT Admin</strong> can delete a PC, and
                            only while it is still <strong className="font-medium text-foreground">Pending</strong> with no
                            forms signed yet. Once any stage is signed, the record is locked and cannot be removed.
                        </p>
                    </div>
                </motion.div>
            </motion.div>
